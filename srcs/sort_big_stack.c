...
void	move_node(t_node **src, t_node **dst, int node_data,
		enum e_stack_id src_id)
{
	int				node_id;
...
void	sort_big_stack(t_node **a, t_node **b)
{
	...
	move_node(a, b, stack_min(*a), e_STACK_A);
	move_node(a, b, stack_max(*a), e_STACK_A);
	while (stack_len(*a) > 5)
	{
